			...
			<section className="py-16 sm:py-24">
				<div className="shell">
					<div className="grid gap-6 lg:grid-cols-[minmax(0,1.12fr)_minmax(260px,0.68fr)] lg:items-end">
						...
						<p className="section-copy max-w-xl lg:justify-self-end">{messages.commands.intro}</p>
					</div>
						...
						{commandGroups.map((group) => (
							<article className="panel p-6 sm:p-7" key={group.title}>
								...
								<div className="mt-3 grid gap-4 lg:grid-cols-[minmax(0,1fr)_180px] lg:items-start">
									<div>
										<h2 className="font-headline text-3xl font-bold tracking-[-0.05em] text-white">{group.title} commands</h2>
										<p className="mt-3 max-w-xl text-sm leading-7 text-muted">{group.copy}</p>
									</div>
									<div className="text-left text-xs font-extrabold uppercase tracking-[0.24em] text-white/45 lg:text-right">{group.items.length} commands</div>
								</div>
					...
					<div className="panel mt-10 p-6 sm:p-7">
						<div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_220px] lg:items-end">
							<div className="max-w-4xl">
								...
								<h2 className="mt-3 font-headline text-3xl font-bold tracking-[-0.05em] text-white">{messages.commands.dashboardTitle}</h2>
							</div>
						</div>
						<div className="mt-6 grid gap-4 lg:grid-cols-3">
							{dashboardGroups.map((group) => (
								<div className="rounded-[1.5rem] border border-white/10 bg-black/25 p-5" key={group.title}>
									<div className="font-headline text-2xl font-bold tracking-[-0.05em] text-white">{group.title}</div>
					...
					<div className="panel mt-10 p-6 sm:p-7">
						<div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-end">
							<div className="max-w-4xl">
								...
								<h2 className="mt-3 font-headline text-3xl font-bold tracking-[-0.05em] text-white">{messages.commands.getStartedTitle}</h2>
								<p className="mt-4 max-w-3xl text-sm leading-7 text-muted">{messages.commands.getStartedCopy}</p>
							</div>
