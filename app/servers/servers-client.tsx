...
export function ServersClient() {
	...
	const loadServerPickerData = async (activeRef: { current: boolean }, options?: { silent?: boolean }) => {
		if (!options?.silent) setIsRefreshing(true);
		try {
			const [botsResponse, guildsResponse] = await Promise.all([apiJson<BotsResponse>('/api/bots'), apiJson<AuthGuildsResponse>('/api/auth/guilds')]);
			...
			if (!activeRef.current) return;
			setGuildsError(error instanceof Error ? error.message : 'Unable to load guilds');
		} finally {
	...
	useEffect(() => {
		...
		const isCacheFresh = cached && Date.now() - cached.refreshedAt < SERVER_CACHE_MAX_AGE_MS;
	...
	const copy =
		connectedGuildCount > 0
			? messages.servers.connectedCopy.replace('{count}', String(connectedGuildCount)).replace('{suffix}', connectedGuildCount === 1 ? '' : 's')
	...
	const filteredGuilds = normalizedSearchQuery
		? visibleGuilds.filter((guild) => guild.name.toLowerCase().includes(normalizedSearchQuery) || guild.guildId.includes(normalizedSearchQuery))
		: visibleGuilds;
	const getGuildPriority = (guild: (typeof filteredGuilds)[number]) => {
		...
		if (guild.canManage && guild.botStates.some((bot) => bot.status === 'invite')) {
			return 2;
		}
		return 3;
	};
