...
const capabilityRows = [
	{
		...
		copy: 'Lunio is not just a slash-command wrapper. It handles live queue control, previous-track replay, autoplay, premium filters, 24/7 voice presence, and custom channel playback surfaces that still feel clean.',
		tags: ['Play', 'Queue', 'Replay', 'Autoplay', '24/7', 'Filters'],
	},
	{
		eyebrow: 'Dashboard',
		title: 'Discord control, but on the web',
		copy: 'Open a guild, switch between connected bots, inspect the queue, control transport, remove queued tracks, change repeat mode, and keep the player feeling live and responsive.',
		tags: ['Multi-bot', 'Live queue', 'Repeat', 'Volume', 'Broker-backed', 'Server aware'],
	},
	{
		eyebrow: 'Guild setup',
		title: 'Server settings that mirror the bot',
		copy: 'Language, request mode, announcement behavior, playlist access, DJ roles, VC restrictions, voice status, custom channel mode, embed mode, default volume, and non-DJ queue limits all live in one place.',
			...
			<section className="relative overflow-hidden border-b border-white/6 py-14 sm:py-20">
				...
				<div className="shell grid gap-8 xl:grid-cols-[minmax(0,1.18fr)_360px]">
					<div className="home-hero panel relative overflow-hidden px-6 py-8 sm:px-10 sm:py-12">
						...
						<p className="mt-8 max-w-2xl text-lg leading-8 text-muted sm:text-xl">{messages.home.heroCopy}</p>
						...
						<div className="mt-12 grid gap-4 border-t border-white/8 pt-8 lg:grid-cols-3">
							{messages.home.featureCards.map((card) => (
								<article className="rounded-[1.6rem] border border-white/8 bg-white/[0.03] p-5" key={card.title}>
									<h2 className="font-headline text-2xl font-bold tracking-[-0.05em] text-white">{card.title}</h2>
					...
					<aside className="grid gap-5">
						<article className="panel relative overflow-hidden p-5 sm:p-6">
							...
							<div className="mt-4 overflow-hidden rounded-[2rem] border border-white/8 bg-[#111] p-4">
								<div className="relative aspect-[0.92] rounded-[1.6rem] border border-white/8 bg-[radial-gradient(circle_at_50%_35%,rgba(0,255,255,0.14),transparent_38%),linear-gradient(180deg,#060707_0%,#0f1111_100%)] p-5">
									...
									<div className="absolute inset-x-6 bottom-8 rounded-[1.4rem] border border-white/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.03)_0%,rgba(255,255,255,0.01)_100%)] p-4">
										<div className="font-headline text-2xl font-bold tracking-[-0.05em] text-white">Midnight City</div>
							...
							<div className="mt-4">
								<p className="font-headline text-2xl font-bold tracking-[-0.05em] text-white">{messages.home.customRequestSurfaces}</p>
								<p className="mt-1 text-sm leading-6 text-muted">{messages.home.customRequestSurfacesCopy}</p>
							</div>
						</article>
						...
						<article className="panel grid gap-4 p-5 sm:p-6">
							<div className="flex items-center justify-between gap-4">
								<div>
									...
									<h2 className="mt-2 font-headline text-3xl font-bold tracking-[-0.05em] text-white">{messages.home.filtersHeading}</h2>
								</div>
				...
				<div className="shell">
					<div className="mb-10 max-w-5xl">
						...
						<div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.7fr)] lg:items-end">
							<div>
								<h2 className="mt-3 max-w-4xl font-headline text-4xl font-bold tracking-[-0.05em] text-white sm:text-5xl">{messages.home.coversHeading}</h2>
							</div>
							<p className="text-base leading-8 text-muted lg:justify-self-end">{messages.home.coversCopy}</p>
						</div>
					</div>

					<div className="grid gap-5">
						{capabilityRows.map((row, index) => (
							<article className={`feature-card ${index === 0 ? 'feature-card-wide' : ''}`} key={row.title}>
								<div className="metric-label text-primary/80">{row.eyebrow}</div>
								<div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
									<div>
										<h3 className="mt-4 max-w-4xl font-headline text-4xl font-bold tracking-[-0.06em] text-white">{row.title}</h3>
