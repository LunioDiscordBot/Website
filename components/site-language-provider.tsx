...
import { getSiteMessages, isSiteLanguage, SITE_LANGUAGE_COOKIE_KEY, SITE_LANGUAGE_OPTIONS, SITE_LANGUAGE_STORAGE_KEY, type SiteLanguage } from '@/lib/site-language';
...
export function SiteLanguageProvider({ children, initialLanguage }: PropsWithChildren<{ initialLanguage: SiteLanguage }>) {
	...
	useEffect(() => {
		...
		const cookieLanguage = document.cookie
			...
			?.split('=')[1];
		const nextLanguage = isSiteLanguage(storedLanguage) ? storedLanguage : isSiteLanguage(cookieLanguage) ? cookieLanguage : initialLanguage;
