...
export function buildSendingCommandFeedback(
  commandType: BrokerCommandType,
  message = "Sending this request to Lunio.",
): CommandFeedback {
  return {
    phase: "sending",
    commandType,
...
export function buildAcceptedCommandFeedback(
...
export function buildCommandFeedbackFromStatus(
...
export function buildSettingsSaveFeedback(
  response: GuildSettingsSaveResponse,
): CommandFeedback {
  return {
    ...
    title: response.command.result.success
      ? "Guild settings applied by Lunio"
      : "Guild settings update failed",
    message: response.command.result.message,
  };
}

export function buildFailedCommandFeedback(
  message: string,
  commandType?: BrokerCommandType,
): CommandFeedback {
  return {
    ...
    commandType,
    title: commandType
      ? `${formatCommandTypeLabel(commandType)} failed`
      : "Request failed",
    message,
    updatedAt: Date.now(),
  };
...
export function buildTimedOutCommandFeedback(
  current?: CommandFeedback,
): CommandFeedback {
  return {
    phase: "timed_out",
